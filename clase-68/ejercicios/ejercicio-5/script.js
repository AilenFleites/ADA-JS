...
jugadoraB con los nombres de cada jugadora respectivamente, y dos arrays de numeros
puntajesA y puntajesB de la misma longitud. La función debe devolver un string con el
nombre de la ganadora o Empate en caso de que no haya ninguna. Para eso, debe comparar las 
mismas posiciones de cada array de puntajes, y sumar puntos a la jugadora correspondiente 
...
const obtenerResultado = (jugadoraA,jugadoraB,puntajesA,puntajesB) =>{
    ...
    for (let i = 0; i <puntajesA.length; i++){
        ...
        jugadoraApuntos++;
        } if (puntajesB[i] > puntajesA[i]) {
        jugadoraBpuntos++;
        } 
    }
    if (jugadoraApuntos>jugadoraBpuntos){
        return "ganadora : " +  jugadoraA 
    } else if (jugadoraApuntos < jugadoraBpuntos) {
