// Tenemos un objeto en la variable banda, con datos de una banda
// (nombre, año de lanzamiento, si sigue en actividad, una foto, los integrantes y una lista de
// sus discos).
// Queremos mostrar:
...
const banda = {
  ...
  const miembros = banda.members;
  const cantidadDiscos = banda.albums.length;
  const discos = banda.albums; // llamo a todos los discos de la banda
  let cantidadTotalCanciones = 0;
  let sumaDuracion = 0;
 
  for (const disco of discos) { //recorro cada disco de la banda y accedo a sus songs y duration
    cantidadTotalCanciones = cantidadTotalCanciones + disco.songs.length;
    sumaDuracion = sumaDuracion + disco.duration;
  }
   promedioDuracion =sumaDuracion/ cantidadTotalCanciones;
